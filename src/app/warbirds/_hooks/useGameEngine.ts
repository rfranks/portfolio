...
import { Cloud, Mountain, Tree, Water } from "@/types/game/environment";
import {
  randomCloud,
  randomMountainRange,
  randomTree,
...
export function useGameEngine() {
  ...
  const doSingleShot = useCallback(
    ...
    [state, play, getImg, changeScore, spawnNapalmEllipse, spawnCrashSmokeOne, makeText, dims],
  );
  ...
  const initLoop = useCallback(() => {
    ...
    const render = ({ deltaMs: frameDeltaMs }: { deltaMs: number }) => {
      ...
      const enemyFlapNow = state.current.enemyFlapTimer >= ENEMY_FLAP_INTERVAL;
      if (enemyFlapNow) {
        ...
        if (!freezeActive && !e.glide) {
          ...
          if (e.loopProgress < 0) {
            ...
            if (e.stepProgress >= 0) {
              ...
              e.rotation = -1 * Math.atan2(dy, state.current.enemySpeed());
      ...
      state.current.groundOffset =
        (state.current.groundOffset + state.current.groundSpeed()) % tileW;
