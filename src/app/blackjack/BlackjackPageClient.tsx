...
const resolveBlackjackWinStreak = (rounds: Array<{ outcome: "loss" | "push" | "win" }>): number => {
  let streak = 0;
  for (let index = rounds.length - 1; index >= 0; index -= 1) {
    if (rounds[index]?.outcome !== "win") {
      break;
    }
    streak += 1;
  }
...
export default function BlackjackPage() {
  ...
  const arcadeRoundSessionActiveRef = React.useRef(arcadeProfile.isSessionActive);
  const processedRoundResultsRef = React.useRef(0);
  ...
  React.useEffect(() => {
    arcadeRoundSessionActiveRef.current = arcadeProfile.isSessionActive;
  }, [arcadeProfile.isSessionActive]);

  React.useEffect(() => {
    if (blackjack.activeRoundNumber <= 0) {
      return;
    }
    if (arcadeRoundSessionActiveRef.current) {
      return;
    }
    startArcadeSession();
    arcadeRoundSessionActiveRef.current = true;
  }, [blackjack.activeRoundNumber, startArcadeSession]);

  React.useEffect(() => {
    const rounds = blackjack.analyticsRounds;
    if (rounds.length < processedRoundResultsRef.current) {
      processedRoundResultsRef.current = rounds.length;
    }
    if (rounds.length <= processedRoundResultsRef.current) {
      return;
    }

    const roundResult = rounds[rounds.length - 1];
    processedRoundResultsRef.current = rounds.length;
    if (!roundResult || !arcadeRoundSessionActiveRef.current) {
      return;
    }

    const positiveRoundScore = Math.max(0, roundResult.netDelta);
    finishArcadeSession({
      completed: true,
      score: positiveRoundScore,
      stats: {
        roundsWon: roundResult.outcome === "win" ? 1 : 0,
        roundsLost: roundResult.outcome === "loss" ? 1 : 0,
        roundsPushed: roundResult.outcome === "push" ? 1 : 0,
        roundNet: roundResult.netDelta > 0 ? roundResult.netDelta : 0,
        roundLoss: roundResult.netDelta < 0 ? Math.abs(roundResult.netDelta) : 0,
        dealerBusts: roundResult.dealerBusted ? 1 : 0,
        bustedHands: roundResult.bustedHands,
        blackjackWins: roundResult.blackjackWin ? 1 : 0,
        roundWinStreak: resolveBlackjackWinStreak(rounds),
      },
    });
    arcadeRoundSessionActiveRef.current = false;
  }, [blackjack.analyticsRounds, finishArcadeSession]);
