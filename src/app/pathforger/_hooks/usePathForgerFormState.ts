...
export function usePathForgerFormState() {
  ...
  const [recentGeneratedProtagonistNames, setRecentGeneratedProtagonistNames] =
    React.useState<string[]>([]);
  const [recentGeneratedPremises, setRecentGeneratedPremises] = React.useState<
    string[]
  >([]);
  ...
  const [selectedPitch, setSelectedPitch] =
    React.useState<PitchSelectionState>("auto");
  ...
  const [modelOptions, setModelOptions] =
    React.useState<string[]>(defaultModelOptions);
  ...
  const [selfieDataUrl, setSelfieDataUrl] = React.useState<
    string | undefined
  >();
  const [selfieName, setSelfieName] = React.useState("");

  const [isRunning, setIsRunning] = React.useState(false);
  const [isGeneratingChapterImages, setIsGeneratingChapterImages] =
    React.useState(false);
  const chapterImageGenerationRunIdRef = React.useRef(0);
  const coverImageGenerationRunIdRef = React.useRef(0);
  const [activeRunAction, setActiveRunAction] =
    React.useState<ActiveRunAction>(null);

  const [errorMessage, setErrorMessage] = React.useState("");
  const [coverImageByPitchKey, setCoverImageByPitchKey] = React.useState<
    Record<string, PathForgerGeneratedImage>
  >({});
  const [pitchInputSignature, setPitchInputSignature] = React.useState<
    string | null
  >(null);
  const [pitchOnlyResult, setPitchOnlyResult] =
    React.useState<PathForgerPitchResult | null>(null);
  const [chapterOnlyResult, setChapterOnlyResult] =
    React.useState<PathForgerChapterResult | null>(null);
  const [result, setResult] = React.useState<PathForgerPipelineResult | null>(
    null,
  );
  ...
  const [imagePromptEditorValue, setImagePromptEditorValue] =
    React.useState("");
  const [activeOptionBranch, setActiveOptionBranch] =
    React.useState<PathForgerBranchChoice | null>(null);
